...
app.post('/guardarActividad', (req, res) => {
    ...
    const query = `
        ...
        (id_paciente, numero_bloqueos, velocidad_media, numero_pasos, duracion, fecha, hora) 
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;

    db.query(query, [idPaciente, bloqueos, velocidadMedia, Ptotal, actividadMin, fecha, hora], (error, results) => {
        if (error) {
